...
- **Delta Parquet comparison** — Delta Parquet part files. The comparison target is the parquet file output, not any downstream database or application instance.
- **CSV comparison** — Simple CSV file outputs and CSV files with trailing control records (header rows, trailer rows with checksums or row counts).
- **Three-tier column classification** — Per-target configuration of EXCLUDED, STRICT, and FUZZY columns (see Section 5).
- **Hash-sort-diff pipeline** — Order-independent comparison via row hashing, sorting by hash, and group-count diff.
- **JSON comparison reports** — One report per comparison target, machine-parseable and human-readable.
- **Configuration** — Per-target configuration with default-strict philosophy and justification requirements. YAML is the reference implementation format; the requirement is structured configuration with comment support.
- **CLI interface** — Accepts a configuration path, LHS data path, RHS data path, and optional output path. Exit codes for scripting.
...
Proofmark compares files to files. There is no database in the comparison loop.
...
There are two reader types: parquet and CSV. Simple CSV and CSV-with-trailer are the same reader with different configuration.
...
**CSV reader:**
- Input: file path
- Configuration: number of header rows (separated from data, preserved in position, compared as literal strings in order), number of trailer rows (separated from data, preserved in position, compared as literal strings in order)
- Everything between header and trailer is data and goes through the hash-sort-diff pipeline
- Header and trailer rows are compared as exact literal string matches, in order, independently from the hash-sort-diff pipeline
- Both header/trailer comparison results and data comparison results appear in the report
...
The same output spread across multiple part files must compare correctly against the same output coalesced into fewer part files.
...
Before the pipeline begins, check the line break style of both LHS and RHS files. If the line break styles differ (e.g., LHS uses CRLF, RHS uses LF):

- Set a **file-level FAIL flag** ("FAIL — line break mismatch").
- Normalize both files to a common line break format internally for row-splitting purposes only.
- **Continue running the full comparison.** The team gets the full picture in one pass — they see the line break problem AND any data mismatches, rather than fixing line breaks, re-running, and discovering additional problems.
- The report includes the match rate from the data comparison plus the line break mismatch flag. Both are visible.

This step does not apply to parquet (binary format; line breaks are not relevant).
...
Both files are read using the same encoding setting (UTF-8 by default, configurable — see Section 9). If either file is not valid in the configured encoding, exit with error (exit code 2).

### Step 2: Schema Validation

Compare the schemas of LHS and RHS. Any schema difference is an **automatic fail** (exit code 1):

- Column count mismatch
- Column name mismatch
- Column type mismatch (including precision differences, e.g., varchar(200) vs. varchar(400) in parquet)
...
For CSV files without embedded schema metadata, column count and header names (if header rows are configured) are validated. Type validation applies to parquet, where the schema is embedded in file metadata.

*(T-10, Revision Log)*

### Step 3: Exclude

Drop all EXCLUDED columns. These columns do not exist from this point forward. They are not hashed, not compared, not present in downstream steps.
...
Group rows by hash value and compare group counts between LHS and RHS:

- **Hash group exists in both LHS and RHS:** Compare counts. If LHS count equals RHS count, all rows in the group are matched. If counts differ, the surplus rows (|lhsCount - rhsCount|) are unmatched.
- **Hash group exists in only one side:** All rows in the group are unmatched (0 matches, counted as surplus).

Within each matched hash group, validate **FUZZY columns** against their configured tolerances (see Section 7). Any FUZZY column value exceeding its tolerance is a mismatch.

**Duplicate row handling:** This is multiset comparison, not set comparison. If the LHS has 3 identical rows (after EXCLUDED column removal and STRICT column hashing), the RHS must also have exactly 3. Row counts per hash group matter.
...
**Requirement:** Every EXCLUDED column must include a documented justification in the configuration. The justification flows into the comparison report. "We excluded it because it didn't match" is not a justification.

### STRICT (Default)

Business-critical fields. Byte-level exact match required. No tolerance, no normalization.

**This is the default.** Any column not explicitly classified as EXCLUDED or FUZZY is STRICT. STRICT columns are hashed for sort ordering and compared via hash group counts.
...
FUZZY columns are excluded from the hash computation but preserved for tolerance comparison within hash groups during the Diff step. This resolves the inherent tension between hashing (which produces wildly different values for minor differences) and tolerance comparison (which needs to evaluate the magnitude of those differences).

**Requirement:** Every FUZZY classification must include a documented justification and an explicit tolerance value and type. "Floating point is weird" is not a justification. "Rounding variance between computation engines, tolerance +-0.01 absolute" is.

### Default-Strict Philosophy

If no column configuration is provided for a comparison target, every column is STRICT (exact match). This is the strictest possible default.

The operator's job is to carve out exceptions and justify each one. The burden of proof is on relaxing the standard, not on tightening it. The evidence package shows: "We compared 47 columns exactly and excluded 3, here's why for each."
...
- Support inline comments (natural place for justifications and audit trail)
- Be human-readable and human-editable
- Be programmatically generatable by automated workflows
- Be less verbose than JSON for readability

YAML is the reference implementation format. The requirement is structured, commented configuration — not a specific serialization format.

### Configuration Defines HOW, Not WHAT

The configuration file defines **how** to compare: reader type, column classifications, tolerances, encoding, threshold. It does NOT contain file paths.

The CLI invocation provides **what** to compare: LHS path, RHS path.

**This separation is critical for reusability.** A single configuration (e.g., `daily_balance.yaml`) is valid for every date's comparison of that target. The same config compares Tuesday's LHS to Tuesday's RHS, then Wednesday's LHS to Wednesday's RHS. If a configuration needs to change between dates, that indicates a problem — the same rigor must be applied consistently across all dates. If a config changes, go back to the start date and re-run all comparisons with the updated config.
...
FUZZY columns support two tolerance modes, configurable per column. There are no hardwired defaults on tolerance type — the person configuring it makes a conscious choice and justifies it.

### Absolute Tolerance

Match condition: `|lhs - rhs| <= tolerance`

Use when the acceptable variance is a fixed value regardless of magnitude (e.g., rounding to the nearest cent).

### Relative Tolerance

Match condition: `|lhs - rhs| / max(|lhs|, |rhs|) <= tolerance`
...
**Byte-level comparison. No null equivalence. No null normalization.** `NULL` is not equal to an empty field, is not equal to `""`, is not equal to `null`. If the bytes are different, it is a mismatch. This is consistent with the default-strict philosophy throughout the system.
...
Both files are read using the same encoding. The default encoding is UTF-8. The encoding is configurable in the comparison target configuration (e.g., `encoding: utf-8`, `encoding: latin-1`).

- If either file is not valid in the configured encoding, Proofmark exits with an error (exit code 2).
- No encoding detection is performed. No encoding normalization is performed.
- Both LHS and RHS are always read with the same encoding setting.

The rewrite process is responsible for producing output in the expected encoding. If the original writes UTF-8, the rewrite must write UTF-8. Encoding mismatches are a rewrite problem, not a comparison tool problem.
...
**MVP:** MD5. It is fast, universally available, and produces excellent distribution for row comparison and sorting. Collision risk is irrelevant — this is a comparison hash, not a security function.
...
JSON. One report per comparison target. JSON serves two audiences: the automated workflow (machine consumer needing structured data to parse pass/fail) and the human reviewer (needing a readable summary for governance). Machine-parseable and human-readable.
...
**Header/trailer comparison (CSV only):**
...
**Control record comparison:**
- Cross-file comparison only: LHS trailer content vs. RHS trailer content as literal strings.
- Proofmark does NOT validate internal consistency (e.g., whether a trailer's row count matches the actual data rows within that file). LHS is the source of truth. Proofmark's job is to prove RHS reproduces what was certified, not to validate the original. This is consistent with the attestation disclaimer.
...
- When correlation confidence is low, the report falls back to presenting unmatched LHS rows and unmatched RHS rows separately.
- Full fuzzy matching across all mismatch scenarios is vendor-build territory.

### Match Percentage Formula

The match percentage represents the proportion of rows that are equivalent across both sides:

- **Per hash group:** matched = min(lhsCount, rhsCount) x 2 (each matched row is counted on both the LHS and RHS side)
- **Surplus per group:** |lhsCount - rhsCount| — these are unmatched rows
- **Rows unique to one side:** Hash groups existing only in LHS or only in RHS contribute 0 matches. All rows in such groups count as surplus.
- **Total rows:** LHS row count + RHS row count (the denominator spans both sides)
- **Match percentage:** totalMatched / totalRows
...
The report shows per-hash-group breakdown with hash value, LHS count, RHS count, and status.
...
- **Platform-specific context.** Job names, orchestration IDs, scheduling metadata — not Proofmark's domain.

### Pass/Fail Logic

- Threshold is in the configuration (e.g., 99.5% match required).
- Default threshold: 100.0% (consistent with default-strict philosophy). Any surplus row = FAIL at default threshold.
- The report always shows ALL mismatches regardless of pass/fail.
- PASS = match percentage >= threshold with all STRICT columns exact and all FUZZY columns within tolerance, and no line break mismatch flag (CSV), and no schema mismatch.
- FAIL = anything else.
...
One config, one comparison, one report.
...
| `1` | Comparison ran, FAIL (including schema mismatch) |
| `2` | Error (bad config, file not found, encoding failure, parse failure) |
...
Exit codes allow automated workflows to script pass/fail decisions without parsing the report.

### What It Does Not Have (By Design)

- **No verbosity flags.** The MVP runs one way. Verbose/quiet/normal is a vendor NFR.
...
- **No CLI flags to override config values.** The config file is authoritative for comparison settings. File paths come from CLI arguments.
...
Evidence package assembly is **out of scope** for Proofmark. Proofmark produces a comparison report. Assembly of that report into a governance evidence package is a QA process concern, not a tool concern.
...
Use a standard parser. Read both files with the same parser configuration. If both were written by the same framework, they should parse the same way.
...
- **Custom application integration pipelines** — Application integration that does not follow standard ETL file output patterns is permanently out of scope. Program stakeholders should be informed that these workflows are out of scope due to the overwhelming complexity of custom integration logic.
...
| BRD Section | Source Decision | Source |
|---|---|---|
| 3.1 Comparison Target | Decision 1 | Design Session 002 |
| 3.2 No Relationships Between Targets | Decision 2 | Design Session 002 |
| 3.3 File vs. File Comparison | Decision 3 | Design Session 002 |
| 3.4 Two Readers | Decision 6, T-5 | Design Session 002, Revision Log |
| 3.5 Parquet Part Files | Decision 4 | Design Session 002 |
| 3.6 LHS / RHS Terminology | S-5 | Revision Log |
| 4. Pipeline: Line Break Pre-Check | T-12 | Revision Log |
| 4. Pipeline: Schema Validation | T-10 | Revision Log |
| 4. Pipeline: Hash (STRICT only) | T-1 | Revision Log |
| 4. Pipeline: Diff (group counts) | T-7 | Revision Log |
| 5. Column Classification (named tiers) | Decision 7, S-4 | Design Session 002, Revision Log |
| 6. Configuration | Decision 11, T-3 | Design Session 002, Revision Log |
| 7. Tolerance Specification | Decision 14 | Design Session 002 |
| 8. Null Handling | Decision 17 | Design Session 002 |
| 9. Encoding Handling | Decision 8, T-11 | Design Session 002, Revision Log |
| 10. Hash Algorithm | Decision 16 | Design Session 002 |
| 11. Report Output | Decision 10, T-2, T-8, T-9 | Design Session 002, Revision Log |
| 12. CLI Interface | Decision 12, T-3 | Design Session 002, Revision Log |
| 14. CSV Dialect | Decision 9 | Design Session 002 |
| 15. SDLC Flow | Decision 18 | Design Session 002 |
