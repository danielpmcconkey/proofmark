...
```python
AppConfig
  paths: PathSettings        # ETL_ROOT token
...
Replaces `{ETL_ROOT}` tokens in a path string with the cached env var value. See [queue-runner.md](queue-runner.md) for how this is used.
